...
import { MVP_RELIC_POOL } from './relics'
...
export function generateRewardBundle(seed: string, tier: EncounterTier, ownedRelics: RelicId[]): RewardBundle {
    ...
    if (tier === 'hallway') {
        ...
        items.push({ kind: 'cards', choices: drawCardChoices(rng) })
        if (rng.random() < 0.4) items.push({ kind: 'potion', potionId: POTION_POOL[rng.int(0, POTION_POOL.length - 1)] })
    } else if (tier === 'elite') {
        items.push({ kind: 'gold', amount: rng.int(25, 35) })
        items.push({ kind: 'cards', choices: drawCardChoices(rng) })
        items.push({ kind: 'relic', relicId: drawRelic(rng, ownedRelics) })
        if (rng.random() < 0.6) items.push({ kind: 'potion', potionId: POTION_POOL[rng.int(0, POTION_POOL.length - 1)] })
    } else if (tier === 'chest') {
        items.push({ kind: 'relic', relicId: drawRelic(rng, ownedRelics) })
        items.push({ kind: 'gold', amount: rng.int(25, 35) })
    }
...
function drawCardChoices(rng: RNG): string[] {
